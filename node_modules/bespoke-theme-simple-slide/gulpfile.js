...
gulp.task('browserify:lib', ['clean:browserify:lib', 'stylus'], function() {
  return gulp.src('lib/bespoke-theme-simple-slide.js')
    .pipe(isDemo ? plumber() : through())
    .pipe(browserify({ transform: ['brfs'], standalone: 'bespoke.themes.simpleSlide' }))
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-theme-simple-slide.min.js'))
    .pipe(uglify())
    .pipe(header(template([
      '/*! <%= name %> v<%= version %> ',
